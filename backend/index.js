...
wss.on('listening', () => console.log(`[NOTICE]: Onu Live on wss://localhost:${wss.options.port}`));
...
function Game() {
    ...
    this.players = {
        ...
        add: player => {
            ...
            game.players.list.forEach(p => p.send("player_joined", { id: player.id, username: player.username }));
    ...
    this.play = (player, card, index) => {
        ...
        game.players.list.forEach(p => {
            ...
            p.send("card_played", { self: false, data: card, index: index, player: player_index });
        });
        ...
        if (card.action == "draw") {
            ...
            const stackable = next.hand.findIndex(x => x.value === last.value && x.action == "draw");
    ...
    this.start = () => {
        ...
        if (card.type == 2) card.colour = colours[Math.floor(Math.random() * colours.length)];

        game.pile.push(card);
        game.players.list.forEach(player => player.send('first_card', card));

        game.startTurn();
        console.log(`${game.id} has just been started with ${game.players.list.length} players!`);
    };
...
function Player(ws) {
    if (!ws) return console.error("[ERROR]: No WebSocket Present to Build Player");
    ...
    this.join = (game_id, username) => {
        if (!game_id || !username) return console.error("[ERROR]: Missing Game Join Data");
    ...
    this.send = (method, body) => {
        ...
        if (!method) return console.error("[ERROR]: Method for WebSocket Message Missing");
    ...
    ws.on('message', message => {
        ...
        if (method == "ready") {
            ...
            players.forEach(player => {
                player.send('status_update', players.map(x => x.ready));
            });

            const ready = players.filter(x => x.ready);
            if (ready.length == players.length && players.length > 1) setTimeout(player.game.start, 2000);
        };
        ...
        if (method == "join_game") {
            if (!body) return;
            if (!body.id || !body.username) return console.error("Missing Information");
            if (player.game) return console.error("User Already in Game");
            const game = locate(body.id);
            if (!game) return player.send('invalid_id');
            body.username = body.username.slice(0, 14);
    ...
    this.generate = (shuffled) => {
        ...
        for (let i = 0; i < 10; i++) {
            colours.forEach(colour => {
                ...
                if (i > 0) deck.cards.push({ type: 0, colour: colour, value: i.toString() });
            });
        };
...
function playable(game, card) {
    ...
    if (game.stack > 0 && card.action != "draw" &&
        card.value !== discarded.value) return false;

    if (card.type == 3 && !card.colour) return false;
    
    if (card.action == discarded.action &&
        card.value === discarded.value) return true;

    if (card.colour == discarded.colour ||
        card.type == 2) return true;
