...
ws.onmessage = (message) => {
  ...
  if (message.method == "turn") {
    ...
    if (message.body.self) {
      ...
      document.querySelector(".game-text").textContent = `It is ${
        players[message.body.index]
      }'s turn!`;
    }
  ...
  if (message.method == "winner") {
    document.querySelector(
      ".game-text",
    ).textContent = `${message.body} has won the game! Refresh to play again!`;
  }
};
  ...
  if (data.type == 2) {
    ...
    colours.forEach((colour, i) => {
      ...
      ele.addEventListener("click", () => {
        ...
        card.setAttribute(
          "class",
          `${card.getAttribute("class")} ${colours[i]}`,
        );
    ...
    if (data.colour)
      card.setAttribute(
        "class",
        `${card.getAttribute("class")} ${data.colour}`,
      );
